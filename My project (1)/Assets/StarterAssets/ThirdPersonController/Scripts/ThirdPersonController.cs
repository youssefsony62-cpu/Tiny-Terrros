...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (isClimbing) return; // Skip all controls while climbing
            ...
            // ✅ Climb input check after movement and input
            HandleClimb();
        }
        ...
        private void UpdateControllerCollider()
        {
            ...
            _controller.height = Mathf.Lerp(_controller.height, targetHeight, CrouchSpeed * Time.deltaTime);
            _controller.center = Vector3.Lerp(_controller.center, targetCenter, CrouchSpeed * Time.deltaTime);
        ...
        }  private void HandleClimb()
        {
            ...
            if (_input.jump && _input.climbAttempt && Grounded)
            {
        ...
        private bool CheckForLedge(out Vector3 ledgePosition)
        {
            RaycastHit hit;
            Vector3 origin = transform.position + Vector3.up * ledgeCheckHeight;
            Vector3 direction = transform.forward;

            if (Physics.Raycast(origin, direction, out hit, ledgeCheckDistance, climbableLayer))
            {
        ...
        private void StartClimb(Vector3 ledgePos)
        {
            isClimbing = true;
            _controller.enabled = false;

            Vector3 climbTarget = ledgePos + new Vector3(0, 1.5f, 0); // Adjust climb height
            StartCoroutine(ClimbToPosition(climbTarget));

            if (_hasAnimator)
                _animator.SetTrigger("Climb"); // Make sure "Climb" exists in Animator
        }
