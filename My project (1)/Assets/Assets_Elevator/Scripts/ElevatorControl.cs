...
public class ElevatorControl : MonoBehaviour
{
   ...
   public Transform    m_weightPulley;
    ...
    float DoorGracePeriod = 3.0f;   // the time we should wait for the doors to open/close before we move the elevator again
	...
	void Update ()
    {
        switch ( currentState )
        {
            case ELEVATOR_STATE.STOPPED: StateStopped(); break;
            case ELEVATOR_STATE.HEADING_TO_FLOOR: StateHeadingToFloor(); break;
            case ELEVATOR_STATE.WAITING_AT_FLOOR: StateWaitAtFloor(); break;
        }
    ...
    void StateHeadingToFloor()
    {
        ...
        if ( m_elevatorTransform.localPosition.y > floorHeightTarget + tolerance )
        {
            ...
            {
                Vector3 goalVector = new Vector3( m_elevatorTransform.localPosition.x, floorHeightTarget, m_elevatorTransform.localPosition.z );
                m_elevatorTransform.localPosition = Vector3.Lerp( m_elevatorTransform.localPosition, goalVector, Time.deltaTime * 3.0f );
            }
            else
                m_elevatorTransform.localPosition = new Vector3( m_elevatorTransform.localPosition.x, m_elevatorTransform.localPosition.y - Time.deltaTime * moveSpeed , m_elevatorTransform.localPosition.z );
        }
    ...
    void MoveCounterWeights()
    {
        ...
        float flCurrentElevatorHeightSubtractBaseFloorHeight = Mathf.Abs( m_elevatorTransform.localPosition.y - flElevatorHeightMin );
        float flCurrentElevatorHeightAsRatio = flCurrentElevatorHeightSubtractBaseFloorHeight  / ( flElevatorHeightMax - flElevatorHeightMin );
        ...
        Vector3 targetWeightPosition = new Vector3 ( m_tranCounterWeights.localPosition.x, targetWeightHeight , m_tranCounterWeights.localPosition.z );
        m_tranCounterWeights.localPosition = Vector3.Lerp( m_tranCounterWeights.localPosition, targetWeightPosition, Time.deltaTime * 4.0f );
    }
...
//            Debug.Log ( "MOVING" + Time.time );
            for ( int i = 0; i < FloorHeights.Length; i++ )
            {
                ...
                else
                {
                    ...
                    if ( m_elevatorTransform.localPosition.y >= floorHeightMin && m_elevatorTransform.localPosition.y <= floorHeightMax )
                    {                   
                        SetFloorMaterial( i );
                        return;
                    }
    ...
    void HandleDoors()
    {
        if ( openDoor )
        {
            m_rightDoor.localPosition = Vector3.Lerp( m_rightDoor.localPosition, m_vecRightDoor_OpenPos, Time.deltaTime * DoorSpeed );
            m_leftDoor.localPosition = Vector3.Lerp( m_leftDoor.localPosition, m_vecLeftDoor_OpenPos, Time.deltaTime * DoorSpeed );
        }
        else
        {
            m_rightDoor.localPosition = Vector3.Lerp( m_rightDoor.localPosition, m_vecRightDoor_ClosePos, Time.deltaTime * DoorSpeed );
            m_leftDoor.localPosition = Vector3.Lerp( m_leftDoor.localPosition, m_vecLeftDoor_ClosePos, Time.deltaTime * DoorSpeed );
        }
    }
